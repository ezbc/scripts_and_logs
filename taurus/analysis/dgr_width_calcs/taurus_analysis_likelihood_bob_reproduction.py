...
def search_likelihoods(mesh):
    ...
    try:
        ...
        if type(av_image_error) != float or \
        	    type(av_image_error) != numpy.float32:
...
def calc_likelihood_hi_av(#hi_cube=None, hi_velocity_axis=None,
        ...
        vel_center_image=None, vel_center_method='CO',
...
def main(region=None):
    ...
    # Derived with K09
    # Velocity widths = 8.00 +41.00/-6.00 km/s
    # Velocity centers = 4.69 +0.00/-0.00 km/s
    # DGRs = 0.14 +0.25/-0.13 10^20 cm^2 mag

    # Derived with Planck
    # Velocity widths = 6.00 +43.00/-4.00 km/s
    # Velocity centers = 4.68 +0.00/-0.00 km/s
    # DGRs = 0.21 +0.18/-0.20 10^20 cm^2 mag

    # Derive CO mask? If co_thres = None, co_thres will be 2 * std(co)
    co_thres = 6.00e10 # K km/s
    ...
    if av_and_co_mask:
        likelihood_filename = 'taurus_nhi_av_likelihoods_' + \
                              'av{0:.1f}mag'.format(av_thres) + \
                              '_bobreproduce_' + av_data_type
        results_filename = 'taurus_likelihood_' + \
                           'av{0:.1f}mag'.format(av_thres) + \
                           '_bobreproduce_' + av_data_type
        if region is not None:
    ...
    if center_vary and width_vary and dgr_vary:
        ...
        if grid_res == 'course':
            ...
            velocity_widths = np.arange(2, 50, 6*0.16667)
            dgrs = np.arange(0.01, 0.4, 1e-2)
        elif grid_res == 'fine':
            ...
            velocity_widths = np.arange(0, 10, 0.16667)
            dgrs = np.arange(0.22, 0.26, 3e-4)
    ...
    print('\nRegion vertices = ', region_vertices)
    ...
    if vel_center_method.lower() == 'co':
        ...
        #import matplotlib.pyplot as plt
        #plt.plot(velocity_axis, hi_spectrum)
        #plt.axvline(velocity_centers[0])
        #plt.show()
    ...
    with open(property_dir + global_property_file + \
              '_region' + str(region) + '_init.txt', 'w') as f:
    ...
    if region is not None:
    	global_property_file += '_region' + str(region)

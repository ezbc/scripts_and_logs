...
def plot_nhi_images(nhi_image=None, nhi_image_inset=None, header=None,
...
def main(dgr=None, vel_range=None, vel_range_type='single', region=None,
    ...
    taurus_prop_file = 'taurus_global_properties_planck_scaled'
    ...
    taurus_property_dir = '/d/bip3/ezbc/taurus/data/python_output/'
    ...
    with open(property_dir + prop_file + '.txt', 'r') as f:
        props = json.load(f)
    with open(taurus_property_dir + taurus_prop_file + '.txt', 'r') as f:
        taurus_props = json.load(f)
    ...
    # create nhi image of taurus only
    nhi_image_taurus = calculate_nhi(cube=hi_cube,
            velocity_axis=velocity_axis,
            velocity_range=taurus_props['hi_velocity_range'],
            header=hi_header,
            noise_cube=hi_noise_cube)

    # extract parameters
    dgr = taurus_props['dust2gas_ratio']['value']

    # Calculate NH2
    nh2_image_taurus = calculate_nh2(nhi_image=nhi_image_taurus,
                              av_image=av_image,
                              dgr=dgr)
    ...
    hi_sd_image_taurus = calculate_sd(nhi_image_taurus,
                                      sd_factor=1/1.25)
    h2_sd_image_taurus = calculate_sd(nh2_image_taurus,
                                      sd_factor=1/0.625)

    # Change WCS coords to pixel coords of images
    props['plot_limit']['wcs'] = (((5, 10, 0), (22, 0, 0)),
                                  ((2, 45, 0), (38, 0, 0)))
    taurus_props['plot_limit']['wcs'] = (((5, 0, 0), (21, 0, 0)),
                                         ((3, 50, 0), (35, 0, 0)))

    # Change WCS coords to pixel coords of images
    props = convert_limit_coordinates(props, header=av_header)

    taurus_props = convert_limit_coordinates(taurus_props, header=av_header)
    ...
    pix = taurus_props['plot_limit']['pixel']
    region_vertices_taurus = np.array(((pix[1], pix[0]),
                               (pix[1], pix[2]),
                               (pix[3], pix[2]),
                               (pix[3], pix[0])
                               ))
    region_vertices_taurus = np.roll(region_vertices_taurus, 1, axis=1)
    ...
    for figure_type in figure_types:
        filename = 'multicloud_nhi_map_taurus_inset' + av_data_type + \
                   '.%s' % figure_type

...
def plot_rh2_hi_vs_h(cloud_dict, fig, font_scale=10, rh2_limits=None,
    ...
    for i, cloud in enumerate(cloud_list):
        ...
        gs1 = GridSpec(2,2)
        gs1.update(left=0.65, right=0.98, bottom=0.69, top=1, wspace=0,
                hspace=0)
        gs2 = GridSpec(2,2)
        gs2.update(left=0.65, right=0.98, bottom=0.36, top=0.63, wspace=0,
                hspace=0)
        gs3 = GridSpec(2,2)
        gs3.update(left=0.65, right=0.98, bottom=0.03, top=0.30, wspace=0,
                hspace=0)
...
def main():
    ...
    for figure_type in figure_types:
        plot_av_images(cloud_dict=cloud_dict,
                       ...
                       show=0,
                       rh2_limits=[0, 79, 1.1*10**-3, 10**2],
                       hi_sd_limits=[0, 79, 1, 8.9])

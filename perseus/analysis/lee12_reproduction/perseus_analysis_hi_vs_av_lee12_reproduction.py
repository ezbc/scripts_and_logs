...
def plot_rh2_vs_h(rh2, h_sd, rh2_error=None, h_sd_error=None, rh2_fit = None,
        ...
        None, show = True, scale=('linear', 'linear'), title = ''):
    ...
    if type(rh2_error) is np.ndarray:
        ...
        rh2_error_nonans = rh2_error * \
                np.ones(rh2[indices].shape)
...
def plot_hi_vs_h_grid(hi_images, h_sd_images, hi_sd_error_images=None,
    ...
    for i in xrange(len(hi_images)):
        ...
        if type(hi_sd_error) is np.ndarray:
            ...
            hi_sd_error_nonans = hi_sd_error * \
                    np.ones(hi_sd[indices].shape)
...
def krumholz_eq_simple(h_sd, phi_CNM, Z, return_fractions=False):

   return krumholz_eq(h_sd,
        phi_CNM=phi_CNM,
        Z=Z, # metallicity
        a=0.2, # ?
        f_diss=0.1, # fraction of absorbing H2 which disociates
...
def fit_krumholz(h_sd, rh2, h_sd_extent, p0 = 10, return_params = False,
    ...
    def chisq(params, h_sd, rh2, rh2_error):
        ...
        rh2_model = krumholz_eq_simple(h_sd, phi_cnm, Z)
    ...
    params.add('Z', value=1.0, min=0.1, max=10, vary=False)
...
def main():
    ...
    # Load data for IC348
    lee12_h_sd_IC348 = np.loadtxt('/d/bip3/ezbc/perseus/data/' + \
            'lee12_reproduction/IC348_h_sd.txt')
    lee12_rh2_IC348 = np.loadtxt('/d/bip3/ezbc/perseus/data/' + \
            'lee12_reproduction/IC348_RH2.txt')

    # location of Av image in Lee+12
    # /d/leffe2/lee/perseus_cloud/Min/R_H2/R_H2_102311/X_factor_COMPLETE/Av_add.rmask.fits

    # location of HI image in Lee+12
    # /d/leffe2/lee/perseus_cloud/Min/R_H2/H2_102311/FIR_based/mask/HI_cdensity.sub.conv4.3.congrid4.3.sub.102311.fits

    # load Planck Av and GALFA HI images, on same grid
    av_data, av_header = load_fits('/d/leffe2/lee/perseus_cloud/Min/R_H2/R_H2_102311/X_factor_COMPLETE/Av_add.rmask.fits',
            return_header=True)

    nhi_image, h = load_fits('/d/leffe2/lee/perseus_cloud/Min/R_H2/H2_102311/FIR_based/mask/HI_cdensity.sub.conv4.3.congrid4.3.sub.102311.fits',
            return_header=True)

    av_data, nhi_image = np.squeeze(av_data), np.squeeze(nhi_image)

    # change N(HI) units to 1e20 cm^-2
    nhi_image /= 1e20
    ...
    for core in cores:
        ...
        if 1:
            ...
            nh2_image = calculate_nh2(nhi_image = nhi_image,
                    av_image = av_data, dgr = dgr)

            nh2_image_error = \
                calculate_nh2_error(nhi_image_error=nhi_image_error,
                    av_image_error=av_error_data, dgr = dgr)
        ...
        #av_error_data_sub = av_error_data[indices]
        av_error_data_sub = av_error_data
        ...
        h_sd_image_error_sub = h_sd_image_error
        rh2_image_sub = rh2_image[indices]
        #rh2_image_error_sub = rh2_image_error[indices]
        rh2_image_error_sub = rh2_image_error[indices]

        #print('nhi max', nhi_image[indices].max())
        #print('hi_sd max', hi_sd_image_sub.max())

        #import matplotlib.pyplot as plt
        #plt.hist(nhi_image[indices])
        #plt.show()
        ...
        h_sd_error_ravel = h_sd_image_error_sub
        ...
        h_sd_error_ravel = h_sd_error_ravel
        ...
        av_image_list.append(av_data_sub)
        av_image_error_list.append(av_error_data_sub)
        ...
        if core == 'IC348':
        	rh2_IC348 = rh2_ravel
        	h_sd_IC348 = h_sd_ravel

    # Extract IC348 core to compare with fitted result from Min
    IC348_pos = core_name_list.index('IC348')
    #rh2_IC348 = rh2_image_list[IC348_pos]
    #h_sd_IC348 = h_sd_image_list[IC348_pos]

    check_results = False
    if check_results:
        print('\n')
        print(rh2_IC348.shape)
        print(lee12_rh2_IC348.shape)
        print(np.median(rh2_IC348))
        print(np.median(lee12_rh2_IC348))

        import matplotlib.pyplot as plt

        fig = plt.figure()
        ax = fig.add_subplot(111)
        ax.hist(lee12_rh2_IC348, alpha=0.5, label='lee12', bins=15)
        ax.hist(rh2_IC348[rh2_IC348 == rh2_IC348], alpha=0.5, label='derived',
                bins=15)
        ax.legend()
        plt.show()
    ...
    for figure_type in figure_types:
        ...
        # Plot results from Lee 12
        plot_rh2_vs_h(lee12_rh2_IC348,
                lee12_h_sd_IC348,
                rh2_error = 0.1,
                h_sd_error = 0.1,
                ...
                filename = 'perseus_rh2_vs_hsd_' +\
                        'lee12_reproduce_IC348.%s' % figure_type,
                title = r'$R_{\rm H2}$ vs. $\Sigma_{\rm HI}$'\
                        + ' of perseus core IC348',
                show = False)

        plot_rh2_vs_h(rh2_IC348,
                h_sd_IC348,
                rh2_error = 0.1,
                h_sd_error = 0.1,
                limits = [0, 80, 10**-3, 10**2],
                ...
                scale = ('linear', 'log'),
                filename = 'perseus_rh2_vs_hsd_' +\
                        'lee12_reproduce_IC348_derived.%s' % figure_type,
                title = r'$R_{\rm H2}$ vs. $\Sigma_{\rm HI}$'\
                        + ' of perseus core IC348',
                show = False)
        ...
        plot_hi_vs_h_grid(hi_sd_image_list,
                ...
                savedir = figure_dir,
                scale = ('log', 'log'),
                filename = 'perseus_hi_vs_h_lee12_reproduce.%s' %\
                        figure_type,
        ...
        plot_av_image(av_image=av_data, header=av_header,
                ...
                savedir=figure_dir,
                filename='perseus_av_cores_map_lee12_reproduce.%s' % \
                        figure_type,
                show=False)

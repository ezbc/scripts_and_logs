...
''' Calculates the N(HI) / Av correlation for the aldobaran molecular cloud.
'''
import matplotlib
matplotlib.use('Agg')
...
def run_analysis(hi_cube=None,
    ...
    if perform_mc:
        ...
        for i in xrange(N_monte_carlo_runs):
            ...
            # the lower alphaG values than for aldobaran mean that aldobaran
...
def load_ds9_core_region(cores, filename_base = 'aldobaran_av_boxes_',
        header=None):
...
def main(verbose=True, av_data_type='planck', regions=None):
    ...
    Script                                          Purpose
    ---------------------------------------------------------------------------
    hi/aldobaran_analysis_core_properties.py           Defines core positions
    av/aldobaran_analysis_av_derive_core_boxes.py      Calculates box sizes
    hi/aldobaran_analysis_hi_av_core_likelihoods.py    Calculates HI velocity
                                                        range
    av/aldobaran_analysis_av_load_regions.py
    ...
    results_filename = '/d/bip3/ezbc/aldobaran/data/python_output/' + \
                       'monte_carlo_results/aldobaran_mc_results_sternberg' + \
                       av_data_type + '_'
    ...
    # Monte carlo results file bases
    results_filename = '/d/bip3/ezbc/aldobaran/data/python_output/' + \
                       'monte_carlo_results/aldobaran_mc_results_krumholz' + \
                       av_data_type + '_'
    ...
    global_property_filename = 'aldobaran_global_properties'
    ...
    figure_dir = '/d/bip3/ezbc/aldobaran/figures/cores/'
    ...
    region_dir = '/d/bip3/ezbc/aldobaran/data/python_output/ds9_regions/'
    ...
    # 'av/aldobaran_analysis_global_properties.txt'
    global_properties_list = []
    for region_name in regions:
        filename = \
                global_property_filename.replace('aldobaran', region_name)
    ...
    with open(core_dir + 'aldobaran_core_properties.txt', 'r') as f:
        cores = json.load(f)

    cores = convert_core_coordinates(cores, h)
    cores = load_ds9_core_region(cores,
                            filename_base = region_dir + \
                                            'aldobaran_av_poly_cores',
                            header = av_header)
    ...
    region_vertices = properties['regions']['aldobaran']['poly_verts']['pixel']
    ...
    if clobber:
        ...
        with open(core_dir + 'aldobaran_core_properties.txt', 'w') as f:
            json.dump(cores, f)
    ...
    for figure_type in figure_types:
        if 0:
            fig_name_rh2 = \
                    'aldobaran_rh2_vs_hsd_panels_{0:s}'.format(av_data_type)
        ...
        fig_name_hivsh = 'aldobaran_hi_vs_h_panels_{0:s}'.format(av_data_type)
        ...
        if 0:
            fig_name_hivsav = \
                    'aldobaran_hi_vs_av_panels_{0:s}'.format(av_data_type)
            print('\nWriting HI vs H figures to\n' + fig_name_hivsav)
...
if __name__ == '__main__':
    main(av_data_type='planck', regions=('aldobaran',))

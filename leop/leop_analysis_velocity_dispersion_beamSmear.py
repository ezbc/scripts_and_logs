...
# Get fits cube
fitsfile = 'leop_beamSmear_model_smooth.fits'
if lappy:
    fitsDir = '/home/elijah/research/leop/data/vla.gmrt/fitsImages/'
if cosmos:
    fitsDir = '/d/bip3/ezbc/leop/data/hi/gipsy/beam_smear_model/'
f = pf.open(fitsDir + fitsfile)
header, cube = f[0].header, f[0].data
#cube = np.ma.array(cube, mask=[cube != cube])
...
cube[cube != cube] = 0.
subCube = cube[:,80:120,80:120] / (0.93 * 1.13*16**2/1.5**2)
...
figureName = 'leop.velDispersionFits.histogram.beamSmear'

    ...
    for i in xrange(len(hi_velocity_axis_list)):
        ...
        if co_velocity_axis_list is not None:
            ax.plot(co_velocity_axis_list[i],
                    co_spectrum_list[i] * 200.,
                    #color='k',
                    label=r'Global Median CO $\times$ 200',
                    drawstyle = 'steps-mid'
                    )
        ...
        if hi_vel_range_list is not None:
...
def main():
    ...
    figure_dir = '/d/bip3/ezbc/california/figures/spectra/'

    clobber_results = 0

    if clobber_results:
        #data_dict = calc_data(ra=(4,29,0), dec=(37, 20, 0))
        data_dict = calc_data(ra=(4,44,0), dec=(33, 05, 0))
        with open(output_dir + 'california_hi_spectra.pickle', 'w') as f:
            pickle.dump(data_dict, f)
    else:
        with open(output_dir + 'california_hi_spectra.pickle', 'r') as f:
    ...
    for figure_type in figure_types:
        ...
        plot_spectra_grid(hi_velocity_axis_list=hi_velocity_axis_list,
                        ...
                        savedir=figure_dir,
                        limits=limits,
                        filename='california_hi_spectrum_velrange.%s' % \
                                figure_type,
                        show=False,
        ...
        plot_spectra_grid(hi_velocity_axis_list=hi_velocity_axis_list,
                        ...
                        savedir=figure_dir,
                        limits=limits,
                        filename='california_hi_spectrum_novelrange.%s' % \
                                figure_type,
                        show=False,
                        spectra_names=spectra_names,
                        #title=r'Global HI Profile' + \

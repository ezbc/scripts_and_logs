...
def main(dgr=None, vel_range=None, vel_range_type='single', region=None,
    ...
    for figure_type in figure_types:
        if region is None:
            if vel_range_type == 'single':
                filename = 'single_vel_range/california_av_model_map_' + \
                    av_data_type + bin_string
                    #'dgr{0:.3f}_'.format(dgr) + \
                    #'{0:.1f}to{1:.1f}kms'.format(vel_range[0], vel_range[1]) + \
                    #'_' + \
            elif vel_range_type == 'multiple':
                filename = 'multiple_vel_range/california_av_model_map_' + \
                           'dgr{0:.3f}'.format(dgr)
                for i in xrange(0, vel_range.shape[0]):
            ...
            filename = 'california_av_model_map_region{0:.0f}'.format(region)
        ...
        if 1:
            ...
            plot_avmod_vs_av((av_model_masked,),
                    ...
                    #scale=('log', 'log'),
                    #scale=('linear', 'linear'),
                    filename='california_avmod_vs_av%s.%s' % (bin_string, figure_type),
                    show = False,
                    std=0.22,
                    )
        ...
        if 0:
            plot_power_spectrum(av_image_masked - av_model_masked,
                filename_prefix='california_av_resid_power_spectrum_' + \

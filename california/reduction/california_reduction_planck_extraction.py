...
def main():
    ...
    temp_dir = '/d/bip3/ezbc/california/data/dust_temp/'
    ...
    if 0:
        ...
        write_data(data_av, header,
                   av_dir + 'california_av_planck.fits')

        write_data(data_av / 10.0, header,
                   av_dir + 'california_av_error_planck.fits')

        # Band maps
        # ---------
    if 0:
        (data, header) = extract_data(datatype = '857')
        write_data(data, header, planck_dir + 'california_planck_857ghz.fits')

        (data, header) = extract_data(datatype = '545')
        write_data(data, header, planck_dir + 'california_planck_545ghz.fits')

        (data, header) = extract_data(datatype = '353')
        write_data(data, header, planck_dir + 'california_planck_353ghz.fits')

    if 0:
        # CO maps
        # -----------------
        (data, header) = extract_data(datatype = 'co_1to0')
        write_data(data, header, co_dir + 'california_co_1-0_planck.fits')

        (data, header) = extract_data(datatype = 'co_1to0_error')
        write_data(data, header, co_dir + 'california_co_1-0_error_planck.fits')

        (data, header) = extract_data(datatype = 'co_2to1')
        write_data(data, header, co_dir + 'california_co_2-1_planck.fits')

        (data, header) = extract_data(datatype = 'co_2to1_error')
        write_data(data, header, co_dir + 'california_co_2-1_error_planck.fits')

        (data, header) = extract_data(datatype = 'co_3to2')
        write_data(data, header, co_dir + 'california_co_3-2_planck.fits')

        (data, header) = extract_data(datatype = 'co_3to2_error')
        write_data(data, header, co_dir + 'california_co_3-2_error_planck.fits')

        (data, header) = extract_data(datatype = 'co')
        write_data(data, header, co_dir + 'california_co_planck.fits')

        (data, header) = extract_data(datatype = 'co_error')
        write_data(data, header, co_dir + 'california_co_error_planck.fits')
    if 1:
        # Temp maps
        # ---------
        (data, header) = extract_data(datatype = 'temp')
        write_data(data, header, temp_dir + 'california_dust_temp.fits')

        (data, header) = extract_data(datatype = 'temp_error')
        write_data(data, header, temp_dir + 'california_dust_temp_error.fits')
